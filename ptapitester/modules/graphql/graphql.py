...
from .modules.supported_methods import IsGraphQL
        ...
        return [
            ...
            {"options": [
                ...
                *_get_available_modules_help(),
                ...
                ["-l", "--loud", "",
                 "Flood GraphQL endpoint with requests to test for rate limiting measures. WARNING: Your IP may get blocked"],
                ...
                ["-dt", "--dump-types", "type1,type2", "Types to dump"]
            ]
            }]
...
class PtGraphQL:
    ...
    def _get_introspection_output(self) -> None:
        if self.args.schema:
            try:
                self.args.schema = json.loads(self.args.schema)
                return
            except JSONDecodeError as e:
                ptprint(f"Could not load schema: {e}", "ERROR", not self.args.json, indent=4)
        ...
        if introspection_data:
            self.args.schema = introspection_data.get("data", {})
    ...
    def run(self) -> None:
        ...
        self.args.headers.update({"Content-Type": "application/json"})

        tests = self.args.tests or _get_all_available_modules()

        if "supported_methods" in tests:
            tests.remove("supported_methods")

        self._check_if_target_runs_graphql()
        if "dos" in tests or "data_grab" in tests:
